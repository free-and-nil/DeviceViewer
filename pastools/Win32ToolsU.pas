...
type
	TDriveType = (dtError, dtRemovable, dtFixed, dtRemote, dtCDROM, dtRamDisk);
...
function CreateProcess_GetCurrentDirectory (const sCmdLine: String;
                                            var sDirectory: String) : Boolean;
...
function FindFileInRootFolder (const sFileName: String;
                               var chDriveLetter: Char;
                               const DT: TDriveTypes = [dtFixed]) : Boolean;
...
function GetDriveInfo (const chDrive: Char; out sName, sFileSystem: String;
					   out dwSerial: DWord;
                       out bSupportsLongFileNames: Boolean) : Boolean;

function GetDriveName (const chDrive: Char; out sName: String) : Boolean;
{$IFDEF SUPPORTS_OVERLOAD}
															           overload;

function GetDriveName (const chDrive: Char) : String; overload;
{$ENDIF}
...
function ReplaceEnvironmentVars (const sPath: String) : String;
...
begin
    ...
    begin
        ...
        try
            ...
            if (CreateProcess_GetCurrentDirectory (sCmdLine,
                                                   sProgramDir{%H-})) then
                pchCurDir := PChar (sProgramDir)
            else pchCurDir := NIL;
            ...
            if (CreateProcess (NIL, szAppName, @SA, @SA, true,
                ...
                try
                    ...
                    repeat
                        ...
                        Win32Check (PeekNamedPipe (hRead, NIL, 0, NIL,
                                    @dwAvailable, NIL));
...
begin
    ...
    if (sCmdLine [1] = '"') then
    begin
        if (Length (sCmdLine) > 1) then
        begin
            iPos := NextPos ('"', sCmdLine, 2);
        ...
        sProgramPath := Copy (sCmdLine, 2, iPos - 2);
    end { if }
    else
    begin
        iPos := Pos (' ', sCmdLine);
        ...
        if (iPos > 0) then
            sProgramPath := Copy (sCmdLine, 1, Pred (iPos))
        else sProgramPath := sCmdLine;
    end; { else }

    sDirectory := ExtractFileDir (sProgramPath);
    Result := sDirectory <> '';
...
begin
{$IFDEF UNICODE}
    Assert (CharInSet (UpCase (chDrive), ['A'..'Z']));
{$ELSE}
    Assert (UpCase (chDrive) in ['A'..'Z']);
{$ENDIF}
...
begin
    ...
    begin
        chDrive := Char (iIndex);
        DriveType := GetDriveType (chDrive);

        if (DriveType in DT) then
        begin
            if (DriveType <> dtFixed) and (DriveType <> dtRemote) then
                if not (IsDiskInDrive (chDrive)) then
                    Continue;

            sFullFileName [1] := chDrive;

            if (FileExists (sFullFileName)) then
            begin
                Result := true;
                chDriveLetter := chDrive;
                Break;
            end; { if }
        end; { if }
    end; { for }
end; { FindFileInRootFolder }
...
{$ENDIF}
    ...
    Result := GetVolumeInformation (PChar (chDrive + ':\'), PChar (sName),
                                    MAX_Path, PDWord (@dwSerial),
...
{$ENDIF}
	...
	Result := GetDriveInfo (chDrive, sName, sFileSystem, dwSerial,
                            bSupportsLongFileNames)
end; { GetDriveName }
...
function GetDriveName (const chDrive: Char) : String; overload;
begin
    if not (GetDriveName (chDrive, Result)) then
        Result := '';
end; { GetDriveName }
{$ENDIF}
...
begin
	Result := Integer (GetPrivateProfileInt (PChar (sSection), PChar (sKey),
                                             iDefault, PChar (sIniName)))
end; { IniReadInt }
...
{$ENDIF}
    ...
    repeat
        iFirstPos := NextPos ('%', Result, iFirstPos);

        if (iFirstPos > 0) then
        begin
            iLastPos := NextPos ('%', Result, iFirstPos + 1);

            if (iLastPos = 0) then
                Break;

            sVarName := Copy (Result, iFirstPos + 1, iLastPos - (iFirstPos + 1));
            sValue := GetEnvironmentVar (sVarName);

            if (sValue <> '') then
            begin
                { Die '%'-Zeichen müssen auch gelöscht werden! }
                Delete (Result, iFirstPos, Length (sVarName) + 2);
                Insert (sValue, Result, iFirstPos);
            end; { if }

            iFirstPos := Succ (iLastPos);
        end; { if }
    until (iFirstPos = 0);
...
{$ENDIF}
    ...
    if (CreateProcess_GetCurrentDirectory (sCmdLine, sProgramDir{%H-})) then
        pchCurrentDir := PChar (sProgramDir)
    else pchCurrentDir := NIL;
