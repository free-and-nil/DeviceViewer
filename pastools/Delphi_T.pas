...
function ReplaceVar (const sStr, sVariable, sValue: String) : String;
...
{$IFDEF DELPHI4_UP}
procedure WriteFile (const sFileName, sText: String;
                     const bEraseExisting: Boolean = false);
{$ELSE}
function WriteFile (const sFileName, sText: String;
                    const bEraseExisting: Boolean
                    const bEraseExisting: Boolean) : Boolean;
{$ENDIF}
...
function ReplaceVar (const sStr, sVariable, sValue: String) : String;
// Pass the name of the variable without enclosing "%"
...
begin
    ...
    iPos := Pos ('%' + LowerCase (sVariable) + '%', LowerCase (sStr));

    if (iPos > 0) then
    begin
        Delete (Result, iPos, Length (sVariable) + 2);
...
{$ENDIF}
    ...
    if (bEraseExisting) then
        if (FileExists (sFileName)) then
{$IFDEF DELPHI4_UP}
            Win32Check (DeleteFile (sFileName));
{$ELSE}
